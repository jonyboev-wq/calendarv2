...
const apiFetch = async <T>(path: string, init?: RequestInit, fallbackMessage?: string): Promise<T> => {
  try {
    ...
    throw new Error(fallbackMessage ?? "Не удалось выполнить запрос");
  }
};
...
export const getSchedule = async (): Promise<ApiSchedule> => {
  if (USE_MOCK_API) {
    return clone(mockState.schedule);
  }
  return apiFetch<ApiSchedule>("/api/schedule", undefined, "Не удалось загрузить расписание");
};

export const createEvent = async (payload: EventPayload): Promise<ApiSchedule> => {
  if (USE_MOCK_API) {
    return mockCreateEvent(payload);
  }
  return apiFetch<ApiSchedule>(
    "/api/events",
    {
      method: "POST",
...
export const completeEvent = async (eventId: string): Promise<ApiSchedule> => {
  if (USE_MOCK_API) {
    return mockCompleteEvent(eventId);
  }
  return apiFetch<ApiSchedule>(
...
export const getProposal = async (payload: EventPayload): Promise<ProposalPreview> => {
  if (USE_MOCK_API) {
    return mockProposal(payload);
  }
  return apiFetch<ProposalPreview>(
    "/api/proposals",
    {
      method: "POST",
      headers: { "Content-Type": "application/json" },
